...
public class ethernet {
	public String ReadEthernetHeader(StringBuilder hexData, int data_size) {
		...
		for(int which_field=itr_start;which_field<EHTERNET_HEADER_LENGTH * 2;which_field++) {
			...
			if(which_field == EHTERNET_ADD_LEN*2 && !parsed_dest_add) {
				...
				System.out.println("ETHER:  Packet Size  = "+data_size+" bytes");
				System.out.println("ETHER:  Destination  = "+header_buffer);
			...
			if(which_field == EHTERNET_ADD_LEN*4) {
				...
				System.out.println("ETHER:  Source       = "+header_buffer);
				header_buffer.setLength(0);
				parsed_dest_add = true;
			}
		...
		System.out.print("ETHER:  Ethertype    = "+header_buffer);

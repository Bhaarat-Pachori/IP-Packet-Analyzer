...
public class tcp {
	...
	public String ReadTCPHeader(StringBuilder hexData, int data_size) {
		...
		for(int which_field=0;which_field<TCPHeader.length();which_field++) {
			...
			if(which_field == DEST_PORT_START) {
				System.out.println("TCP:  Source port = "+Integer.parseInt(parsing_header.toString(), 16));
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			...
			if(which_field == SEQ_START) {
				System.out.println("TCP:  Destination port = "+Integer.parseInt(parsing_header.toString(), 16));
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			...
			if(which_field == ACK_START) {
				...
				System.out.println("TCP:  Sequence Number = "+BigInteger.valueOf(Long.parseLong(parsing_header.toString(), 16)));
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			...
			if(which_field == ACK_END+2) {
				//BigInteger big1 = new BigInteger(parsing_header.toString(), 16);
				System.out.println("TCP:  Acknowledgement Number = "+BigInteger.valueOf(Long.parseLong(parsing_header.toString(), 16)));
				// We multiply by 4 as data offset is 32bit word field.
				System.out.println("TCP:  Data offset = "+data_offset*4);
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			...
			if(which_field == WINDOW_SIZE_START) {
				...
				for(int iterator=0;iterator<9;iterator++) {
					...
					if(iterator == 0) {
						System.out.println("TCP:  Flags = 0x"+flag_buf.charAt(2)+flag_buf.charAt(3));
					}
				...
				if(bit_shifter == 0) {
					System.out.println(print_buffer.toString());
					pktanalyzer.reset_parsing_buffer(parsing_header);
		 ...
		 if(which_field == CHKSUM_START) {
			System.out.println("TCP:  Window Size = "+Integer.parseInt(parsing_header.toString(), 16));
			pktanalyzer.reset_parsing_buffer(parsing_header);
		 }
		 ...
		 if(which_field == URG_PTR_START) {
			 System.out.println("TCP:  Checksum = 0x"+parsing_header.toString());
			 pktanalyzer.reset_parsing_buffer(parsing_header);
		 }
		 ...
		 if(which_field == URG_PTR_END+1 && urg_status) {
			 System.out.println("TCP:  Urgent pointer = "+Integer.parseInt(parsing_header.toString(), 16));
		 }
		...
		System.out.println("TCP:  Options = "+options_size+" bytes");
		System.out.println("TCP: ");
		...
		print_data(val, data_buf, hexData.length() - pktanalyzer.total_header_size);
		// End Parsing Header
		return "no header";
	}
	public static void print_data(int val, StringBuilder data_buf, int len) {
		...
		for(int itr=skip_bytes; itr<len; itr++) {
			...
			if(itr % 4 == pattern) {
				if(octet_counts == 7 && itr !=8) {
					print_data.append("\n");
					System.out.print(print_data.toString());

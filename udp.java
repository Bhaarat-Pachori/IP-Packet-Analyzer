...
public class udp {
	...
	public String ReadUDPHeader(StringBuilder hexData, int data_size) {
		...
		for(int which_field=0;which_field<UDP_LEN*2;which_field++) {
			...
			if(which_field == (DEST_PORT_START)-1) {
				...
				System.out.println("UDP:  Source port = "+val);
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			...
			if(which_field == (LEN_START)-1) {
				int val = Integer.parseInt(parsing_header.toString(), 16);
				System.out.println("UDP:  Destination port = "+val);
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			// parsing Length
			...
			if(which_field == (CHKSUM_START)-1) {
				int val = Integer.parseInt(parsing_header.toString(), 16);
				System.out.println("UDP:  Length = "+val);
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			// parsing Checksum
			...
			if(which_field == (CHKSUM_END)-1) {
				System.out.println("UDP:  Checksum = "+"0x"+parsing_header.toString());
				pktanalyzer.reset_parsing_buffer(parsing_header);
			}
			// parsing data
			...
			if(which_field == (UDP_LEN*2)-1) {
				System.out.println("UDP:  Data: ("+DATA_LEN+ " bytes)");
				System.out.print("UDP:  ");
				int octet_counts = 0;
				for(int itr=0;itr<parsing_header.length();itr++) {
					System.out.print(parsing_header.charAt(itr));
					if(itr == 3 || itr % 4 == 3) {
						System.out.print(" ");
						octet_counts++;
					}
